...
let occupated = []
  ...
  function createCircle(frame, i) {
    ...
    for (let i=0; i< occupated.length; i++){
      ...
      if ((occupated[i][0] <= left) &  (occupated[i][1] <= top)) {
          if (((left - occupated[i][0]) < occupated[i][2]) || 
            ((top-occupated[i][1])< occupated[i][2])) {
              overlap =true
              break
          }
          else {
      ...
      if ((occupated[i][0] >= left) & (occupated[i][1] <= top)) {
          if ((top - occupated[i][1]) > occupated[i][2]) {
              s = sizeMax
          }
          else {
              s =  occupated[i][0] - left
          }
      }

      if ((occupated[i][0] <= left) & (occupated[i][1] >= top)) {
          if ((occupated[i][0] - left) > occupated[i][2]) {
              s = sizeMax
          }
          else {
              s = occupated[i][1] - top
      ...
      if ((occupated[i][0] >=left) &  (occupated[i][1] >=top)) {
          s = Math.max((occupated[i][0] - left), (occupated[i][1] - top))
      }
    ...
    if (!overlap) {
      size = Math.floor(Math.min.apply(sizeMax, setD))  
      console.log('new: ', left, top, size)
